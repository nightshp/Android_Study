...
//		return userService.updateUsers(user) > 0 ? "ok" : "error";
    ...
    @ResponseBody
    public User login(@RequestParam String userName,@RequestParam String userPwd)
    {
        return userService.selectUsername(userName, userPwd);
    }
    ...
    public String logout(SessionStatus sessionStatus, @RequestParam(value = "pn", defaultValue = "1") Integer pn,
        Model model)
    {
...
//		return "<script>window.location.href='../selectAllMenu';</script>";
     ...
     *
     * @param user 用户实体对象
     * @return PageHelper包装的结果集
     */
    @RequestMapping("/getAll")
    @ResponseBody
    public PageHelper<User> getAll(@RequestBody User user)
    {
    ...
    public String upload(MultipartFile file, Model model, Integer id, User user)
        throws IOException
    {
        //获取文件名
    ...
    @ResponseBody
    public String updateUser(@ModelAttribute User user, @RequestParam(value = "file") MultipartFile file)
        throws IOException
    {
        upload(user, file);
...
//		return "forward:/getAll";
    ...
    private void upload(@ModelAttribute User user, @RequestParam("file") MultipartFile file)
        throws IOException
    {
        //获取文件名
        String filename = file.getOriginalFilename();
    ...
    @ResponseBody
    public String insertUser(@ModelAttribute User user, @RequestParam(value = "file") MultipartFile file)
        throws IOException
    {
        upload(user, file);

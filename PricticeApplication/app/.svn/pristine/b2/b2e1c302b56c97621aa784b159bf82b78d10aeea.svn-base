 ...
 * Date: 2019/3/1 9:18
 */
public class RecommendView extends View implements View.OnClickListener, View.OnFocusChangeListener {
    // 上下文
    private Context mContext;

    // 影片列表数据
    private List<VideoMini> mVideoList;
    ...
    public RecommendView(Context context, List<VideoMini> videoList, int index) {
        super(context);
        mContext = context;
    ...
    private void initView() {
        ...
        frameLayout = rootView.findViewById(R.id.fl_recommend_item);
        frameLayout.setOnFocusChangeListener(this);
        frameLayout.setOnClickListener(this);
        setData(mVideoList, index);
    }

    /**
     * 设置数据
     */
    public void setData(List<VideoMini> videoList, int index) {
        RequestOptions requestOptions = new RequestOptions().placeholder(R.mipmap.movie_default_icon_comm).dontAnimate();
        VideoMini video = videoList.get(index);

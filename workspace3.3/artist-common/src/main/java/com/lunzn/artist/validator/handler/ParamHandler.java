package com.lunzn.artist.validator.handler;
...
import com.lunzn.artist.util.ReflectUtil;
import com.lunzn.artist.util.StringUtil;
import com.lunzn.artist.validator.IValidatable;
...
public class ParamHandler implements Handler
{
        ...
        throws Exception
    {
        ...
        else if (IMsgRequestData.class.isAssignableFrom(fieldClazz))
        {
            // 业务封装类，递归校验封装类对象
            IMsgRequestData fieldObj = (IMsgRequestData)JSON.parseObject(JSON.toJSONString(fieldValue), fieldClazz);
            ...
            catch (com.lunzn.artist.exception.InnerException e)
            {
                e.printStackTrace();
            }
            return;
